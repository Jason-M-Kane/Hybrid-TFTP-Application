...
int updateClientEntry(int clientIndex,int desiredConnections, 
                      unsigned short firstPort, int mode)
{
    ...
    connectionDetails[clientIndex].mode = mode;
...
/* Purpose: Removes a connection from the table.                             */
...
int removeConnection(char* ipaddress)
{
    ...
    /* Delete file from Peer Cache folder */
    sprintf(tchar,".\\PeerCache\\%s_cached.dat",ipaddress);
    DeleteFile(tchar);
...
/* Purpose: Resets the timeout on a connected peer.                          */
/* Input:   The ip address of the client to have its timer reset.            */
/* Returns: 0 on success, -1 on error (cant find entry).                     */
/*****************************************************************************/
int resetConnectionTimeout(char* ipaddress)
{
    int index;
...
/*****************************************************************************/
/* Function: isClientinP2PMode                                               */
/* Purpose: Returns mode related information.                                */
/* Input:   The index of the client.                                         */
/* Returns: 0 if the client is in P2P mode, -1 if it is in client/server.    */
/*****************************************************************************/
int isClientinP2PMode(int clientIndex)
{
    if(connectionDetails[clientIndex].mode == CS_CONNECT)
    {
        /* Client/Server Mode */
        return -1;
    }
