...
/* Program Information                                                       */
/*---------------------------------------------------------------------------*/
/* What it does: a P2P hybrid tftp server that will run concurrently.        */
/* It is based on the protocol developed by Robert Hernandez.                */
/* It takes 3 parameters: port to run on, max # simult connections, file     */
/* share path.                                                               */
/*                                                                           */
/* Useage:  tftp_server.exe port max_conn sharepath                          */
/*****************************************************************************/
...
int testDisconnect(char* buff, struct sockaddr_in client);
int sendDisconnectReply(SOCKET sock, struct sockaddr_in to);
int testKeepalive(char* buff, struct sockaddr_in client);
int sendKeepaliveReply(SOCKET sock, struct sockaddr_in to);
int testDirRqst(char* buff);
int sendDirectoryListing(SOCKET sock, struct sockaddr_in to, 
                         char* fsListing);
int testConnRqst(char* buff, int* mode);
...
void begin_server(unsigned short port, int maximumConnections, char* share)
{
    char* fileSystemListing, *p2pFileSystemListing;
    ...
    int rtn, rval, size, clientIndex, numDesiredConnections,mode,dirReqType;
    HANDLE hThread = NULL;
    DWORD ThreadId = 0;
    listeningSocket = INVALID_SOCKET;      /* Init */

    /* Prepare the file system contents */
    fileSystemListing = p2pFileSystemListing = NULL;
    ...
    /* Create the P2P Filesystem */
    cacheDirListing(fileSystemListing,"127.0.0.1");
    if(createP2PFileSystemListing(&p2pFileSystemListing) < 0)
    {
        reportError("Error creating P2P File System Listing, aborting.\n",0,0);
        return;
    }
    ...
    while(1)
    {
        ...
        if(FD_ISSET(listeningSocket, &etherfds))
        {
            ...
            //Disconnect Rqst Packet
            if(testDisconnect(inbuff,from) > 0)
            {
                sendDisconnectReply(listeningSocket, from);
                continue;
            }

            //Keepalive Rqst Packet
            if(testKeepalive(inbuff,from) > 0)
            {
                sendKeepaliveReply(listeningSocket, from);
                continue;
            }
            ...
            if((dirReqType=testDirRqst(inbuff)) > 0)
            {
                ...
                {
                    //For P2P recreate directory listing before sending
                    //so that it is up-to-date
                    free(p2pFileSystemListing);
                    p2pFileSystemListing = NULL;
                    destroyP2PFSlinkedList();
                    createP2PFileSystemListing(&p2pFileSystemListing);

                    sendDirectoryListing(listeningSocket, from, p2pFileSystemListing);
                }
                else if(dirReqType == CS_DIRLISTING)
                {
            ...
            /*************************************/
            if( (numDesiredConnections=testConnRqst(inbuff,&mode)) != -1)
            {
                sendConnRqstReply(listeningSocket, from, numDesiredConnections,mode);

                /* If you are in P2P mode, get the directory listing and add it to the database */
                if(mode == P2P_CONNECT)
                {
                    char* destBuffer = NULL;
                    if(getDirListing(inet_ntoa(from.sin_addr),&destBuffer, TRUE) < 0)
                    {
                        reportError("Error retrieving client directory.",0,0);
                        continue;
                    }
                    cacheDirListing(destBuffer,inet_ntoa(from.sin_addr));
                    free(destBuffer);
                    destBuffer = NULL;
                }
                continue;
            ...
            data->enableP2PMode = isClientinP2PMode(clientIndex);
...
int sendAutoDetectReply(SOCKET sock, struct sockaddr_in to)
{
    ...
    currentClients = (char)(getMaxConnections() - getNumAvailConnections());
    tmp[15] = maxClients;
...
/*Purpose:   Determines if the packet is a disconnect packet.               */
/*Input: the buffer containing the packet, client info.                     */
/*Return: -1 if not a valid disconnect, 1 if it is a valid disconnect.      */
/****************************************************************************/
int testDisconnect(char* buff, struct sockaddr_in client)
{
    char tmp[32];
    ...
    if(memcmp(tmp,buff,18) != 0)
    {
        return -1;
    }

    /* Match Detected */
    removeConnection(inet_ntoa(client.sin_addr));
...
/****************************************************************************/
/*Function:  sendDisconnectReply                                            */
/*Purpose:   Sends a disconnect reply packet.                               */
/*Input: the socket to send the packet on, a sockaddr_in struct to          */
/*       determine where to send the packet.                                */
/*Return: 0 on success.                                                     */
/****************************************************************************/
int sendDisconnectReply(SOCKET sock, struct sockaddr_in to)
{
    ...
    /* Create the disconnect reply packet */
    memset(tmp,0,32);
    id = htons(OPC_DISCON_RPLY);
    memcpy(&tmp[0],&id,2);
    strcpy(&tmp[2],"TFTP_DISCONNECTR");

    /* Send the packet */
    udp_send(sock,tmp,19,to);
...
/*Purpose:   Determines if the packet is a keepalive packet.                */
/*Input: the buffer containing the packet, client info.                     */
/*Return: -1 if not a valid keepalive, 1 if it is a valid keepalive.        */
/****************************************************************************/
int testKeepalive(char* buff, struct sockaddr_in client)
{
    char tmp[32];
    ...
    if(memcmp(tmp,buff,17) != 0)
    {
        return -1;
    }

    /* Match Detected */
    resetConnectionTimeout(inet_ntoa(client.sin_addr));
...
/****************************************************************************/
/*Function:  sendKeepaliveReply                                             */
/*Purpose:   Sends a keepalive reply packet.                                */
/*Input: the socket to send the packet on, a sockaddr_in struct to          */
/*       determine where to send the packet.                                */
/*Return: 0 on success.                                                     */
/****************************************************************************/
int sendKeepaliveReply(SOCKET sock, struct sockaddr_in to)
{
    ...
    /* Create the disconnect reply packet */
    memset(tmp,0,32);
    id = htons(OPC_KEEPA_RPLY);
    memcpy(&tmp[0],&id,2);
    strcpy(&tmp[2],"TFTP_KEEPALIVER");
                      ...
                      int mode)
{
    ...
    if( (clientIndex=searchForConnectedClient(inet_ntoa(to.sin_addr))) != -1)
    {
		...
		if( (allowedConnections = 
            updateClientEntry(clientIndex, desiredConn, ntohs(to.sin_port),mode)) < 0)
		{
            deny = TRUE;
        }
		else
    ...
    else
    {
        if( (allowedConnections=(char)addConnection(desiredConn, 
            ntohs(to.sin_port), inet_ntoa(to.sin_addr),mode)) < 0)
        {
            deny = TRUE;
        }
    }
...
/*Return: nothing                                                           */
/****************************************************************************/
DWORD WINAPI Execute_P2P_SendRqst(LPVOID sockdata)
{
    SOCKET local;
    int len,fail,x;
    struct sockaddr_in remote;
