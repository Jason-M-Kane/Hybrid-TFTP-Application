...
int createFileSystemListing(char* rootpath, char** fileSystemBuffer)
{
    ...
    if(addFileSystemItem(rootpath, 1, G_numFileSystemLevels, 0) < 0)
    {
    ...
    else
    {
        ...
        if(createFSListing(fileSystemBuffer,G_fileSystemLvl_HeadPtrs) < 0)
        {
...
void destroyFSlinkedList()
{
    ...
    {
        tempBegin = G_fileSystemLvl_HeadPtrs[x];
        ...
        else if(fileattrib & FILE_ATTRIBUTE_DIRECTORY)
        {
            ...
            if((currentlevel+1) <= 256)
            {
                if(addFileSystemItem(next_path,
                    currentlevel+1, MAX_FS_LEVELS, currentItem->ID) < 0)
                {
                    return -1;
                }
            }
...
int createFSListing(char** fileSystemBuffer, listItem** ptrHeadPtrs)
{
    ...
    /*********************************************************************/
    while(ptrHeadPtrs[x] != NULL)
    {
        level = x+1;
        currentItem = ptrHeadPtrs[x];
